...
int grid[M][N] = { 0 };//we will use M,N for dimensions 

//declaring tile size here
int ts = 18;
...
public:
    ...
    void move()// update position based on GLs
    {
        x += dx;
...
void drop(int y, int x) // Grid tile ko update 
{
...
class saveHighScore :public HighScore {
public:
    saveHighScore() {}
    saveHighScore(int highScore) {     //2
...
void saveHighScore(int highScore) {     //2
    std::ofstream file("highscore.txt");
    if (file.is_open()) {
        file << highScore;
        file.close();
...
int main()
{
    ...
    if (!font.loadFromFile("E:\\repos\\survive\\arial.ttf")) {
        std::cout << "font unavailable2";
    }
    ...
    Texture t1, t2, t3;
    t1.loadFromFile("E:\\repos\\survive\\images\\tiles.png");

    t2.loadFromFile("E:\\repos\\survive\\images\\gameover.png");

    t3.loadFromFile("E:\\repos\\survive\\images\\op.png");
